...
struct KernelResponse cmdChangeRule() {
	...
	empty.code = ERROR_CODE_EXIT;
	// 规则序号（以1开始）
	printf("order of the rule to be changed [like 1 for the head]: ");
	scanf("%d", &key);
	// 规则名
	printf("rule name [max len=%d, -1 for no change]: ", MAXRuleNameLen);
	scanf("%s",name);
	if(strlen(name)==0 || strlen(name)>MAXRuleNameLen) {
		printf("name too long or too short.\n");
		return empty;
	...
	// 源IP
	printf("source ip and mask [like 127.0.0.1/16, -1 for no change]: ");
	scanf("%s",saddr);
	// 源端口
	printf("source port range [like 8080-8031 or any, -1 for no change]: ");
	scanf("%s",sport);
	if(strcmp(sport, "-1") == 0){
	...
	// 目的IP
	printf("target ip and mask [like 127.0.0.1/16, -1 for no change]: ");
	scanf("%s",daddr);
	// 目的端口
	printf("target port range [like 8080-8031 or any, -1 for no change]: ");
	scanf("%s",dport);
	if(strcmp(sport, "-1") == 0){
	...
	// 协议
	printf("protocol [TCP/UDP/ICMP/any, -1 for no change]: ");
	scanf("%s",protoS);
	if(strcmp(protoS,"TCP")==0)
		proto = IPPROTO_TCP;
	else if(strcmp(protoS,"UDP")==0)
	...
	// 动作
	printf("action [1 for accept, 0 for drop, 2 for no change]: ");
	scanf("%d",&action);
	// 是否记录日志
	printf("is log [1 for yes, 0 for no, 2 for no change]: ");
	scanf("%u",&log);
	printf("result:\n");
...
void wrongCommand() {
	...
	printf("commands: rule <add | del | ls | change | default> [del rule's name]\n");
